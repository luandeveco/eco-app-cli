...
export default function detailedReportServer(
  pdf,
  Movement,
  dateFormatted,
  messengerData,
  ...
  const formatValue = value => {
    let formattedValue = parseFloat(value).toFixed(2).toString().replace('.', ',');
  ...
  const createSectionText = (title, data, count) => {
    ...
    const total = data.reduce((sum, movement) => sum + parseFloat(movement.valor_prev || 0), 0);
    const formattedTotal = formatValue(total);

    return (
      `\n${title.toUpperCase()} (${count}) - R$ ${formattedTotal}\n\n` +
      data
        .map(
          movement =>
    ...
    const templatePDF = () => {
      ...
      const createTwoColumnLayout = (returnedData, receivedData, title) => {
        const totalReturned = returnedData.reduce(
          (sum, movement) => sum + parseFloat(movement.valor_prev || 0),
          0,
        );
